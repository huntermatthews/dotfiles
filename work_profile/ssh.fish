...
# NOTE: order matters here -- we want to add the card first so its primary 
# when we are logged into a server without a .ssh/.config that specifies a key.
alias ssh-add-all "ssh-card-add ; ssh-add --apple-load-keychain"
